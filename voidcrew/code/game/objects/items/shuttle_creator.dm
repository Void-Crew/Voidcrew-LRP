...
/obj/item/shuttle_creator/proc/shuttle_create_docking_port(atom/target, mob/user)
	...
	port.current_ship.unlocked_helm = TRUE
	...
	port.register()
	message_admins("[ADMIN_LOOKUPFLW(user)] created a new shuttle with a [src] at [ADMIN_VERBOSEJMP(user)] ([num_customs] customs, limit is [CONFIG_GET(number/max_shuttle_count)])")
	log_game("[key_name(user)] created a new shuttle with a [src] at [AREACOORD(user)] ([num_customs] customs, limit is [CONFIG_GET(number/max_shuttle_count)])")
	return TRUE
