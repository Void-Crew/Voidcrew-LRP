...
/mob/living/simple_animal/hostile/megafauna/marg/proc/bloodgrab(turf/T, handedness)
	...
	for(var/mob/living/L in T)
		if(!faction_check_mob(L))
			if(L.stat != CONSCIOUS)
				to_chat(L, "<span class='userdanger'>[src] glitches you through the floor!</span>")
				playsound(T, 'sound/magic/enter_blood.ogg', 100, TRUE, -1)
				var/turf/targetturf = get_step(src, dir)
				L.forceMove(targetturf)
				playsound(targetturf, 'sound/magic/exit_blood.ogg', 100, TRUE, -1)
...
/mob/living/simple_animal/hostile/megafauna/marg/proc/blood_warp()
	...
	if(pools.len)
		...
		found_bloodpool = pick(pools)
	if(found_bloodpool)
		visible_message("<span class='danger'>[src] t-poses and glitches sideways through the floor...</span>")
		playsound(get_turf(src), 'sound/magic/enter_blood.ogg', 100, TRUE, -1)
		forceMove(get_turf(found_bloodpool))
		playsound(get_turf(src), 'sound/magic/exit_blood.ogg', 100, TRUE, -1)
		visible_message("<span class='danger'>And teleports back out!</span>")
		blood_enrage()
		return TRUE
	return FALSE
...
/mob/living/simple_animal/hostile/megafauna/marg/proc/blood_enrage_end(var/newcolor = rgb(149, 10, 10))
	update_approach()
	change_move_delay()
	remove_atom_colour(TEMPORARY_COLOUR_PRIORITY, newcolor)

/mob/living/simple_animal/hostile/megafauna/marg/proc/change_move_delay(var/newmove = initial(move_to_delay))
	move_to_delay = newmove
	set_varspeed(move_to_delay)
	handle_automated_action() // need to recheck movement otherwise move_to_delay won't update until the next checking aka will be wrong speed for a bit
...
/mob/living/simple_animal/hostile/megafauna/marg/bullet_act(obj/projectile/P)
	if(BUBBLEGUM_IS_ENRAGED)
		visible_message("<span class='danger'>[src] deflects the projectile; [p_they()] can't be hit with puny projectiles while filled with divine fury!</span>", "<span class='userdanger'>You deflect the projectile!</span>")
		playsound(src, pick('sound/weapons/bulletflyby.ogg', 'sound/weapons/bulletflyby2.ogg', 'sound/weapons/bulletflyby3.ogg'), 300, TRUE)
		return BULLET_ACT_BLOCK
	return ..()
...
/mob/living/simple_animal/hostile/megafauna/marg/Bump(atom/A)
	if(charging)
		...
		if(isliving(A))
			var/mob/living/L = A
			L.visible_message("<span class='danger'>[src] glitches through [L]!</span>", "<span class='userdanger'>[src] speedmerges you with the floor!</span>")
			src.forceMove(get_turf(L))
